...
НЕ перечисляй все задачи! Сделай осмысленное резюме что было сделано.

ВАЖНО про стадию проекта:
- Указывай стадию ТОЛЬКО если она ЯВНО видна из задач
- "ушёл на релиз" - ТОЛЬКО если есть задача со словами "релиз", "отправил в стор", "опубликовал"
- "на тестах" - ТОЛЬКО если есть задачи про тесты, дебаг
- Если стадия НЕ ЯСНА из задач - НЕ ПИШИ её, просто опиши что сделано
- НЕ ВЫДУМЫВАЙ стадию!
...
Пример ПРАВИЛЬНОГО выхода (стадия НЕ указана, т.к. не ясна из задач):
<a href="https://link.com">719. MirrorBeam</a> - Добил UI, сделал хромакаст фичи

680. GroupFusion - Сделал основной функционал

Правила:
- НЕ пиши "Отчет за..." - заголовок добавится автоматически
- ОДНА строка на проект с КРАТКИМ резюме
- Стадию пиши ТОЛЬКО если она 100% ясна из задач
- НЕ ВЫДУМЫВАЙ информацию которой нет в задачах
- НЕ используй markdown (**, *, `)
- Используй HTML только для ссылок: <a href="url">текст</a>
